...
public class GUIMessage : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        manager = GetComponent<ZEDManager>();
    }

    private void OnEnable()
    {
        ZEDManager.OnZEDReady += Ready;
      //  ZEDUpdater.OnZEDDisconnected += ZEDDisconnected;
    ...
    {
        warning.SetActive(true);
        if (image != null) image.transform.parent.gameObject.SetActive(false);
    ...
    void Update()
    {
        if (!init)
        {
            sl.ERROR_CODE e = ZEDManager.LastErrorMessageInit;
