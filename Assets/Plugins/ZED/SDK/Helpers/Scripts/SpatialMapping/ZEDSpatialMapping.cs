...
public class ZEDSpatialMapping
{
    ...
    private void UpdateMeshMainthread(bool spreadUpdateOverTime = true)
    {
        ...
        lock (lockScanning)
        {
            ...
            for (; indexUpdate < zedSpatialMapping.NumberUpdatedSubMesh; indexUpdate++)
            {
                ...
                zedSpatialMapping.SetMesh(indexUpdate, ref verticesOffset, ref trianglesOffset, ref uvsOffset, display, holder.transform, updatedTexture);
    ...
    public void SwitchDisplayMeshState(bool newDisplayState)
    {
        ...
        {
            submesh.Value.o.GetComponent<MeshRenderer>().enabled = newDisplayState;
        }
    }
    ...
    /// <summary>
    /// Save the mesh as an obj file and the area database as an area file. It can be quite time comsuming if you mapped a large area. It will stop the saptial mapping.
    /// </summary>
    ...
    /// </summary>
    public void ReshapeChunks()
    {        
        lock (lockScanning)
        {
            zedSpatialMapping.ReshapeChunks();
            zedSpatialMapping.ResizeMesh();
            zedSpatialMapping.RetrieveMesh();
            meshUpdated = true;
        }
    ...
    private void Stop()
    {
        ...
        else
        {
            ...
            ClearMeshes();
            filterThread = new Thread(() => PostProcessMesh(filterParameters != sl.FILTER.NONE));
            filterThread.Start();
            stopRunning = true;
    ...
    private class ZEDSpatialMappingHelper
    {
        ...
        /// </summary>
        public ZEDSpatialMapping.Chunk CreateNewMesh(int i, Material meshMat, bool display, Transform holder)
        {
            //The chunk
        ...
        public void SetMesh(int indexUpdate, ref int verticesOffset, ref int trianglesOffset, ref int uvsOffset, bool display, Transform holder, bool updatedTex)
        {
            ZEDSpatialMapping.Chunk subMesh;
            int updatedIndex = UpdatedIndices[indexUpdate];
            if (!chunks.TryGetValue(updatedIndex, out subMesh))
            {
                subMesh = CreateNewMesh(updatedIndex, materialMesh, display, holder);
            }
        ...
        {
            if (filterParameters != sl.FILTER.NONE)
            {
        ...
        public void ReshapeChunks()
        {
            zedCamera.ReshapeChunks(MAX_SUBMESH, numVerticesInSubmesh, numTrianglesInSubmesh, ref numUpdatedSubmesh, UpdatedIndices, ref numVertices, ref numTriangles, MAX_SUBMESH);
        }
