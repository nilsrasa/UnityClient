...
public class WheelchairSimConnector : MonoBehaviour
{
    ...
    private void PublishData(int leftTicks, int rightTicks)
    {
        ...
        float linearSpeed = ((left + right) / 2) * _coefficient;
        float angularSpeed = ((left - right) / _wheelbase) * _coefficient;
        _rosLocomotionDirect.PublishData(new Vector2(angularSpeed, linearSpeed));
    }
}
