...
Make shure to update the camera [firmware](https://support.theta360.com/en/download/firmware/s/) to version 01.82+ (tested with version 01.90).

### Charging Ricoh Theta S camera before use

The camera should be charged before use (also when connected by wire). Otherwise, the camera might emit some audio beeps and shut down. When charging, the switch of the camera will be solid red light as shown in the picture.
...
### Start camera streaming mode

The streaming mode is started by pressing the top ⏻ and bottom 📷 buttons at the same time. Then the *Live* blue text will light.

## USB camera network sharing

Use e.g. [VirtualHere](http://www.virtualhere.com) to share the camera over USB to another machine.
